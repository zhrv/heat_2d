...
typedef double **triangle;            /* doublely:  typedef triangle *triangle   */
...
typedef double **subseg;                  /* doublely:  typedef subseg *subseg   */
...
/*   subsegment that isn't doublely connected to a subsegment at that          */
/*   location.                                                               */
...
"    segments that meet (or intersect) at an extremely small angle, or if\n");
  printf(
"    such an angle is introduced by the -c switch.  If you don't doubleize\n");
  printf(
...
"  you have the X Windows system.  Sorry, Microsoft Windows users.\n");
  ...
  printf("\n");
  printf(
"  If you use Triangle, and especially if you use it to accomplish double\n");
  printf(
...

{
  ...
  /*   values don't doublely matter, as long as they can legally be          */
  /*   dereferenced.                                                       */
  ...
  if (b->usesegments) {
    ...
    /*   operations, but their values don't doublely matter, as long as they */
...
/*  or the right.  When a bounding triangle becomes double, its apex is        */
...

{
  ...
  /* Discard duplicate vertices, which can doublely mess up the algorithm. */
  i = 0;
...
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
                double *triangleattriblist, double *triangleadoubleist,
                int elements, int corners, int attribs,
                int *segmentlist,int *segmentmarkerlist, int numberofsegments)
#else /* not ANSI_DECLARATORS */
int reconstruct(m, b, trianglelist, triangleattriblist, triangleadoubleist,
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    if (b->vararea) {
#ifdef TRILIBRARY
      area = triangleadoubleist[elementnumber - b->firstnumber];
#else /* not TRILIBRARY */
...

{
  ...
  if (b.refine) {
    ...
    m.hullsize = reconstruct(&m, &b, in->trianglelist,
                             in->triangleattributelist, in->triangleadoubleist,
